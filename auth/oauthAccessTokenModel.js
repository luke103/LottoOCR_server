...
function getAccessToken(bearerToken, callback) {
    ...
    accessTokensDBHelper.getUserIDFromBearerToken(bearerToken)
        .then(userID =>{
                ...
                if (userID)
                    return createAccessTokenFrom(userID.user_id)
                else
                    throw new Error ("User not found")
        })
        .then(accessToken =>{console.log(accessToken); callback(false, accessToken)})
        .catch(error => {console.log(error);callback(true , null)})
}
